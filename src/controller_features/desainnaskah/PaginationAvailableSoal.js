...
export default class PaginationAvailableSoal{
    ...
    listenerSelecting(){
        ...
        cari.oninput = (e)=>{
            if(e.target.value !==''){
                let soalfilter = data.banksoal.filter(m=>Object.entries(m).filter(([k,v])=>v.toString().toLowerCase().indexOf(cari.value.toLowerCase())>-1).length!==0);
                let assignData = Object.assign({}, this.data, {banksoal:soalfilter});
                this.eventPreviewSoalCari(assignData)
            }else{
                this.listenerSelecting();
            }
        }
    }
    eventPreviewSoalCari(soalfilter){
        
        const data = soalfilter
        const span= document.querySelector('.mbs_infohalaman');
        const awal = document.querySelector('.mbs_awal');
        const akhir = document.querySelector('.mbs_akhir');
        const next = document.querySelector('.mbs_next');
